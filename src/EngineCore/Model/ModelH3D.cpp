...
void Model::LoadTextures(void)
{
	...
	for (uint32_t materialIdx = 0; materialIdx < m_Header.materialCount; ++materialIdx)
	{
		...
		{
			MatTextures[1] = littleEngine::Graphics::TextureManager::LoadFromFile(std::string(pMaterial.texDiffusePath) + "_specular", true);
			if (!MatTextures[1]->IsValid())
		...
		{
			MatTextures[3] = littleEngine::Graphics::TextureManager::LoadFromFile(std::string(pMaterial.texDiffusePath) + "_normal", false);
			if (!MatTextures[3]->IsValid())
				MatTextures[3] = littleEngine::Graphics::TextureManager::LoadFromFile("default_normal", false);
		}
