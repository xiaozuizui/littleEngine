...
uint32_t Color::R11G11B10F(bool RoundToEven) const
{
	...
	R.f = littleEngine::Math::Clamp(m_value.f[0], 0.0f, kMaxVal) * kF32toF16;
	G.f = littleEngine::Math::Clamp(m_value.f[1], 0.0f, kMaxVal) * kF32toF16;
	B.f = littleEngine::Math::Clamp(m_value.f[2], 0.0f, kMaxVal) * kF32toF16;
...
uint32_t Color::R9G9B9E5() const
{
	...
	float r = littleEngine::Math::Clamp(m_value.f[0], 0.0f, kMaxVal);
	float g = littleEngine::Math::Clamp(m_value.f[1], 0.0f, kMaxVal);
	float b = littleEngine::Math::Clamp(m_value.f[2], 0.0f, kMaxVal);

	// Compute the maximum channel, no less than 1.0*2^-15
	float MaxChannel = littleEngine::Math::Max(littleEngine::Math::Max(r, g), littleEngine::Math::Max(b, kMinVal));
