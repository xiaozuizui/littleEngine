...
D3D12_CPU_DESCRIPTOR_HANDLE littleEngine::Graphics::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE Type, UINT Count )
{
	return g_DescriptorAllocator[Type].Allocate(Count);
}

void littleEngine::Graphics::Initialize(void)
{
	...
	ASSERT_SUCCEEDED(dxgiFactory->CreateSwapChainForHwnd(g_CommandManager.GetCommandQueue(), littleEngine::EngineCore::g_hWnd, &swapChainDesc, nullptr, nullptr, &s_SwapChain1));
#else // UWP
...
void littleEngine::Graphics::CompositeOverlays(GraphicsContext& Context)
{
...
void littleEngine::Graphics::PreparePresentHDR()
{
	
}

void littleEngine::Graphics::PreparePresentLDR(void)
{
...
void littleEngine::Graphics::Present(void)
{
	/*if (littleEngine::Graphics::g_bEnableHDROutput)
		PreparePresentHDR();
	else
		PreparePresentLDR();*/
	PreparePresentLDR();
	...
	littleEngine::Graphics::s_SwapChain1->Present(0, 0);
