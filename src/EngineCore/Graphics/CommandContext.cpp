...
CommandContext* ContextManager::AllocateContext(D3D12_COMMAND_LIST_TYPE Type)
{
	...
	//littleEngine::Utility::Printf("AvailableContext %d\n", AvailableContexts.size());
	//littleEngine::Utility::Printf("Context Pool %d\n\n", sm_ContextPool[Type].size());
...
{
	CommandContext* NewContext = littleEngine::Graphics::g_ContextManager.AllocateContext(D3D12_COMMAND_LIST_TYPE_DIRECT);
	NewContext->SetID(ID);
	//if (ID.length() > 0)
...
{
	ASSERT(BufferData != nullptr && littleEngine::Math::IsAligned(BufferData, 16));
	DynAlloc TempSpace = m_CpuLinearAllocator.Allocate(NumBytes, 512);
	SIMDMemCopy(TempSpace.DataPtr, BufferData, littleEngine::Math::DivideByMultiple(NumBytes, 16));
...
void CommandContext::FillBuffer(GpuResource& Dest, size_t DestOffset, DWParam Value, size_t NumBytes)
{
	...
	SIMDMemFill(TempSpace.DataPtr, VectorValue, littleEngine::Math::DivideByMultiple(NumBytes, 16));
	CopyBufferRegion(Dest, DestOffset, TempSpace.Buffer, TempSpace.Offset, NumBytes);
}
...
void CommandContext::InitializeBuffer(GpuResource& Dest, const void* BufferData, size_t NumBytes, size_t Offset)
{
	...
	SIMDMemCopy(mem.DataPtr, BufferData, littleEngine::Math::DivideByMultiple(NumBytes, 16));
...
{
	ASSERT(VertexData != nullptr && littleEngine::Math::IsAligned(VertexData, 16));

	size_t BufferSize = littleEngine::Math::AlignUp(NumVertices * VertexStride, 16);
...
{
	ASSERT(IndexData != nullptr && littleEngine::Math::IsAligned(IndexData, 16));

	size_t BufferSize = littleEngine::Math::AlignUp(IndexCount * sizeof(uint16_t), 16);
...
void GraphicsContext::SetDynamicSRV(UINT RootIndex, size_t BufferSize, const void* BufferData)
{
	ASSERT(BufferData != nullptr && littleEngine::Math::IsAligned(BufferData, 16));
	DynAlloc cb = m_CpuLinearAllocator.Allocate(BufferSize);
	SIMDMemCopy(cb.DataPtr, BufferData, littleEngine::Math::AlignUp(BufferSize, 16) >> 4);
