...
ID3D12DescriptorHeap* DescriptorAllocator::RequestNewHeap(D3D12_DESCRIPTOR_HEAP_TYPE Type)
{
	...
	ASSERT_SUCCEEDED(littleEngine::Graphics::g_Device->CreateDescriptorHeap(&Desc, IID_PPV_ARGS(&pHeap)));
	sm_DescriptorHeapPool.emplace_back(pHeap);
	return pHeap.Get();
}


//申请descriptor时会按照256一组，如果Count>256，则会开辟新的一组
D3D12_CPU_DESCRIPTOR_HANDLE DescriptorAllocator::Allocate(uint32_t Count)
{
	if (m_CurrentHeap == nullptr || m_RemainingFreeHandles < Count)
	{
		...
		if (m_DescriptorSize == 0)
			m_DescriptorSize = littleEngine::Graphics::g_Device->GetDescriptorHandleIncrementSize(m_Type);
	}
...
//
// User 的最大Descripotor自定,没有pool
void UserDescriptorHeap::Create(const std::wstring& DebugHeapName)
{
	ASSERT_SUCCEEDED(littleEngine::Graphics::g_Device->CreateDescriptorHeap(&m_HeapDesc, IID_PPV_ARGS(m_Heap.ReleaseAndGetAddressOf())));
#ifdef RELEASE
	(void)DebugHeapName;
#else
	...
	m_DescriptorSize = littleEngine::Graphics::g_Device->GetDescriptorHandleIncrementSize(m_HeapDesc.Type);
