...
LinearAllocationPage* LinearAllocatorPageManager::RequestPage()
{
	...
	while (!m_RetiredPages.empty() && littleEngine::Graphics::g_CommandManager.IsFenceComplete(m_RetiredPages.front().first))
	{
...
void LinearAllocatorPageManager::FreeLargePages(uint64_t FenceValue, const std::vector<LinearAllocationPage*>& LargePages)
{
	...
	while (!m_DeletionQueue.empty() && littleEngine::Graphics::g_CommandManager.IsFenceComplete(m_DeletionQueue.front().first))
	{
...
LinearAllocationPage* LinearAllocatorPageManager::CreateNewPage(size_t PageSize)
{
	...
	ASSERT_SUCCEEDED(littleEngine::Graphics::g_Device->CreateCommittedResource(&HeapProps, D3D12_HEAP_FLAG_NONE,
...
DynAlloc LinearAllocator::Allocate(size_t SizeInBytes, size_t Alignment)
{
	...
	const size_t AlignedSize = littleEngine::Math::AlignUpWithMask(SizeInBytes, AlignmentMask);

	if (AlignedSize > m_PageSize)
		return AllocateLargePage(AlignedSize);

	m_CurOffset = littleEngine::Math::AlignUp(m_CurOffset, Alignment);
