...
void RootSignature::Finalize(const std::wstring& name, D3D12_ROOT_SIGNATURE_FLAGS Flags)
{
	...
	size_t HashCode = littleEngine::Utility::HashState(&RootDesc.Flags);
	HashCode = littleEngine::Utility::HashState(RootDesc.pStaticSamplers, m_NumSamplers, HashCode);
	...
	for (UINT Param = 0; Param < m_NumParameters; ++Param)
	{
		...
		if (RootParam.ParameterType == D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE)
		{
			...
			HashCode = littleEngine::Utility::HashState(RootParam.DescriptorTable.pDescriptorRanges,
			...
			HashCode = littleEngine::Utility::HashState(&RootParam, 1, HashCode);
	}
	...
	if (firstCompile)
	{
		...
		ASSERT_SUCCEEDED(littleEngine::Graphics::g_Device->CreateRootSignature(1, pOutBlob->GetBufferPointer(), pOutBlob->GetBufferSize(),
