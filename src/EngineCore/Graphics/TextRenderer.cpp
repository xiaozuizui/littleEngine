	...
	namespace Graphics
	{
		namespace TextRenderer
		{
			...
			public:
				...
				float GetAntialiasRange(float size) const { return Math::Max(1.0f, size * m_AntialiasRange); }
			...
			GraphicsPSO s_TextPSO[2];    // 0: R8G8B8A8_UNORM no hdr  1: R11G11B10_FLOAT
	...
	: m_Context(CmdContext)
{
	...
	m_VSParams.ViewportTransform = littleEngine::Math::Vector4(twoDivW, -twoDivH, -vpX * twoDivW - 1.0f, vpY * twoDivH + 1.0f);
...
void TextContext::DrawString(const std::wstring& str)
{
	...
	TextVert* vbPtr = littleEngine::Math::AlignUp((TextVert*)stackMem, 16);
