...
ID3D12DescriptorHeap* DynamicDescriptorHeap::RequestDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE HeapType)
{
	...
	//littleEngine::Utility::Printf("DynamicDescriptor %d\n", sm_AvailableDescriptorHeaps[idx].size());
	//littleEngine::Utility::Printf("DynamicPool %d\n\n", sm_DescriptorHeapPool[idx].size());

	while (!sm_RetiredDescriptorHeaps[idx].empty() && littleEngine::Graphics::g_CommandManager.IsFenceComplete(sm_RetiredDescriptorHeaps[idx].front().first))
	{
